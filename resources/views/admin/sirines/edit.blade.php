...
<div class="form-card">
  <form method="POST" action="/admin/sirines/{{ $sirine->id }}" enctype="multipart/form-data">
    @csrf
    @method('PUT')
    <div class="form-group">
      <label for="name">Nama Sirine</label>
      <input type="text" id="name" name="name" value="{{ old('name', $sirine->name) }}" required>
    </div>
    <div class="form-group">
      <label>Foto Utama Saat Ini</label>
      <div class="current-image-wrap">
        <img id="imagePreview" src="{{ $sirine->image ? Storage::url($sirine->image) : asset('placeholder.png') }}" class="preview-image is-visible" alt="{{ $sirine->name }}">
      </div>
    ...
    <div class="form-group">
      ...
      @if(!empty($sirine->gallery_images))
        <div class="media-preview-grid">
          @foreach($sirine->gallery_images as $image)
    ...
    <div class="form-group">
      ...
      @if(!empty($sirine->gallery_videos))
        <div class="media-preview-grid">
          @foreach($sirine->gallery_videos as $video)
            <div class="media-preview-card">
      ...
      @endif
      <label for="gallery_videos" class="media-preview-label">Ganti Video Sirine (opsional)</label>
    ...
    <div class="form-group">
      ...
      <textarea id="description" name="description">{{ old('description', $sirine->description) }}</textarea>
    ...
    <div class="form-group">
      ...
      <textarea id="english_description" name="english_description">{{ old('english_description', $sirine->english_description) }}</textarea>
      <div class="help">Isi agar user yang memilih English melihat deskripsi sirine yang sesuai.</div>
    </div>
    <div class="form-group">
      <label for="price">Harga (Rp)</label>
      <input type="number" id="price" name="price" value="{{ old('price', $sirine->price) }}" required>
    </div>
    <div class="form-group">
      <label for="shopee_url">Link Shopee</label>
      <input type="url" id="shopee_url" name="shopee_url" value="{{ old('shopee_url', $sirine->shopee_url) }}" placeholder="https://shopee.co.id/...">
    </div>
    <div class="form-group">
      <label for="tiktokshop_url">Link TikTok Shop</label>
      <input type="url" id="tiktokshop_url" name="tiktokshop_url" value="{{ old('tiktokshop_url', $sirine->tiktokshop_url) }}" placeholder="https://www.tiktok.com/...">
    </div>
    <div class="form-group">
      <label for="tokopedia_url">Link Tokopedia</label>
      <input type="url" id="tokopedia_url" name="tokopedia_url" value="{{ old('tokopedia_url', $sirine->tokopedia_url) }}" placeholder="https://www.tokopedia.com/...">
      <div class="help">Kosongkan link yang tidak dipakai.</div>
    </div>
    <div class="form-actions">
      <button type="submit" class="btn btn-primary">Update Sirine</button>
...
<script>
  ...
  if (editImageInput && editPreview) {
    editImageInput.addEventListener('change', function (event) {
      ...
      if (!file) {
        editPreview.src = '{{ $sirine->image ? Storage::url($sirine->image) : asset('placeholder.png') }}';
        return;
      }
